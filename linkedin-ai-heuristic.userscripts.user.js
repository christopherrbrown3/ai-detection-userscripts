...
//
// ==UserScript==
// @name         LinkedIn AI Content Heuristic (Safari Userscripts)
// @version      0.11.1
// @description  Adds a small AI-likelihood label next to LinkedIn feed posts (local heuristic only). Safari-optimized for the Userscripts extension.
...
`;
  ...
  function extractFeatures(rawText, context) {
    ...
    const isComment = context && context.kind === 'comment';
  ...
  const MODEL_POST = {
    ...
    weights: {
      ...
      buzzPer100w: 1.0,
      templatePer100w: 0.9,
      discoursePer100w: 0.7,
      bigramRepeatRatio: 1.1,
      trigramRepeatRatio: 0.7,
      sentenceStarterRepeatRatio: 0.6,
      typeTokenRatio: -1.1,
      hapaxRatio: -0.5,
      sentenceLenCV: -0.8,
      avgSentenceLen: 0.6,
      listMarkerCount: 0.45,
      colonPer100w: 0.25,
      commaPer100w: 0.18,
      exclamationsPer100w: 0.12,
      emojiPresent: 0.08,
      topWordShare: 0.25
    }
  };
  ...
  const MODEL_COMMENT = {
    ...
    weights: {
      ...
      bigramRepeatRatio: 0.95,
      trigramRepeatRatio: 0.55,
  ...
  function computeEvidence(metrics) {
    ...
    let evidence = 'LOW';
    const medMin = isComment ? 20 : 35;
    const highMin = isComment ? 55 : 90;
...
Disclaimer: Heuristic estimate; not proof.`;
  ...
  function findPostText(postEl) {
    // LinkedIn feed markup changes frequently; keep selectors broad.
    const candidates = postEl.querySelectorAll(
      'div.update-components-text, div.update-components-update-v2__commentary, ' +
      'div.feed-shared-update-v2__description, div.feed-shared-update-v2__commentary, ' +
      'div.occludable-update, div.occludable-update__commentary, ' +
      'span[dir="ltr"], span.break-words, span[dir="auto"], div[dir="auto"], ' +
      // comments
      'span.comments-comment-item__main-content, ' +
      'div.comments-comment-item__main-content, ' +
      'div.comments-comment-item__content, ' +
      'span.comments-comment-item__content, ' +
      'div.comments-comment-entity__content, ' +
      'span.comments-comment-entity__content, ' +
  ...
  function placeBadge(postEl, badge) {
    // Try to place near the author name or header
    const header = postEl.querySelector(
      '.feed-shared-actor__container, .update-components-actor__meta, ' +
      '.update-components-actor__title, header'
    );
    if (header) {
      const target = header.querySelector('span, div') || header;
      target.appendChild(badge);
      return true;
    }
    // Fallback: append near top of post
  ...
  function scanComments() {
    const comments = document.querySelectorAll(
      'li.comments-comment-item, div.comments-comment-item, article.comments-comment-item, ' +
      'li.comments-comments-list__comment-item, div.comments-comments-list__comment-item, ' +
      'div.comments-comment-entity, article.comments-comment-entity, ' +
      'li.update-components-comment, div.update-components-comment, ' +
      'div.comments-comment-item__main, div.comments-comment-item__content'
    );
  ...
  function processComment(commentEl) {
    ...
    const textHost = commentEl.querySelector(
      'span.comments-comment-item__main-content, ' +
      'div.comments-comment-item__main-content, ' +
      'div.comments-comment-item__content, ' +
      'span.comments-comment-item__content, ' +
    ...
    if (textHost && textHost.parentElement) {
      ...
      // Fallback: top of the comment element
